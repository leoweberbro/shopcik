...
from tgbot.data.config import CHANNEL_ID
...
@router.callback_query(F.data == "check_sub_channel")
async def check_sub_channel_f(call: CallbackQuery, bot: Bot, state: FSM, arSession: ARS):
    ...
    try:
        ...
        if member.status in ["member", "administrator", "creator"]:
            await call.message.answer(
                ded("""
                    ✅ <b>Вы успешно подписаны на канал!</b>
                    
                    🎉 Теперь вы можете продолжить пользоваться ботом.
                    Нажмите <b>/start</b>, чтобы вернуться в главное меню.
                """),
                disable_web_page_preview=True
            )
        else:
            await call.answer("❌ Вы не подписаны на канал. Подпишитесь, чтобы продолжить.", show_alert=True)
    except Exception as e:
        await call.answer("❗ Ошибка проверки подписки. Попробуйте позже.", show_alert=True)
